...
void Sketchpad::Draw() const {
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      ...
      if (image_.GetValue(row, col) == 1) {
        ci::gl::color(ci::Color::gray(0.3f));
      } else {
        ci::gl::color(ci::Color("white"));
      }

      vec2 pixel_top_left = top_left_corner_ + vec2(static_cast<double>(col) * pixel_side_length_,
...
void Sketchpad::HandleBrush(const vec2& brush_screen_coords) {
  ...
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      vec2 pixel_center = {static_cast<double>(col) + 0.5, static_cast<double>(row) + 0.5};

      if (glm::distance(brush_sketchpad_coords, pixel_center) <=
          brush_radius_) {
        
        image_.SetValue(row, col, 1);
      }
    }
  }
}
